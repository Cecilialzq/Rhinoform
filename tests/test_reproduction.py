...
from tools.reproduce import parser, verify_paper_experiments
...
def test_every_paper_experiment_has_public_evidence_paths() -> None:
    root = Path(__file__).resolve().parents[1]
    paths = ReproductionPaths.resolve(repo_root=root, environ={})
    report = verify_paper_experiments(paths)
    assert report["status"] == "PASS_PAPER_EXPERIMENT_COVERAGE"
    assert report["experiments"] == 7
    assert len(report["notebooks"]) == 3
    assert report["publication_figures"]["files"] >= 40
    assert report["runtime_benchmarks"]["files"] >= 5
