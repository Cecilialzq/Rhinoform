...
def relocate_comparable_model_config(
    recorded: dict[str, object], live: dict[str, object]
) -> dict[str, object]:
    """Return the frozen config with only its mount-dependent region path moved."""
    if not comparable_model_config(recorded, live):
        raise ValueError("LAMM structural configuration mismatch")
    if "region_ids_file" not in recorded or "region_ids_file" not in live:
        raise ValueError("LAMM region_ids_file is required for safe relocation")
    relocated = dict(recorded)
    relocated["region_ids_file"] = live["region_ids_file"]
    return relocated


def require_frozen_result_artifact(
    results_root: Path,
    artifact: Path,
    *,
    manifest_relative_path: str | None = None,
) -> str:
    """Bind a relocated artifact to the immutable canonical result-tree manifest."""
    root = Path(results_root).resolve()
    path = Path(artifact).resolve()
    manifest_path = root / "RESULT_TREE_MANIFEST.json"
    if not valid_sha256_sidecar(manifest_path):
        raise RuntimeError(f"Invalid canonical result-tree manifest: {manifest_path}")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("status") != "CANONICAL_RESULT_TREE_FROZEN":
        raise RuntimeError("Canonical result-tree manifest has an unexpected status")
    if manifest_relative_path is None:
        try:
            relative = path.relative_to(root).as_posix()
        except ValueError as exc:
            raise RuntimeError(f"Artifact is outside the frozen result tree: {path}") from exc
    else:
        requested = Path(manifest_relative_path)
        if requested.is_absolute() or ".." in requested.parts:
            raise RuntimeError(f"Unsafe result-tree manifest path: {manifest_relative_path}")
        relative = requested.as_posix()
    matches = [row for row in manifest.get("artifacts", []) if row.get("path") == relative]
    if len(matches) != 1:
        raise RuntimeError(f"Artifact is missing or duplicated in result-tree manifest: {relative}")
    if not valid_sha256_sidecar(path):
        raise RuntimeError(f"Invalid frozen artifact or sidecar: {path}")
    actual = sha256_file(path)
    expected = str(matches[0].get("sha256", ""))
    expected_bytes = int(matches[0].get("bytes", -1))
    if actual != expected or path.stat().st_size != expected_bytes:
        raise RuntimeError(
            f"Artifact differs from result-tree manifest: {relative} "
