...
def main() -> int:
    ...
    signature = sha256_json(
        {
            "schema": "rbsr_projection_none_logit_calibration_v1",
            ...
            "pairs": pairs,
            "strict_rule": "roi_rmse < matched_ridge and normal_flip_pct <= matched_ridge",
        }
    )
    ...
    for array_path in source_arrays:
        ...
        if resumed:
            ...
            condition, _, _, _, _, _, _ = pair_conditions(
                by_id,
                saved_pairs,
                base["source_pca"],
                base["cond_mean"],
            ...
            if not np.array_equal(zero_prediction, ridge_prediction):
