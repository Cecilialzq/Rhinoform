...
def main() -> int:
    ...
    proposer_kind = str(
        rbsr.get("residual_proposer", rbsr.get("args", {}).get("residual_proposer", "cvae"))
    )
    if proposer_kind not in {"cvae", "field"}:
        raise ValueError(f"Unknown RB-SR residual proposer: {proposer_kind}")
    proposer_state_key = f"{proposer_kind}_state_dict"
    if proposer_state_key not in base:
        raise RuntimeError(f"Base package lacks {proposer_state_key}")
    ...
    calibration_freeze_status: str | None = None
    ...
    if args.calibration_freeze:
        ...
        calibration_freeze_status = str(calibration_freeze.get("status"))
    ...
    if proposer_kind == "cvae":
        ...
        proposer_is_cvae = True
    else:
        proposer = NeuralFieldDeformer(
            vertex_features_np.shape[1],
            condition.shape[1],
            hidden=int(base_args["hidden"]),
        )
        proposer_is_cvae = False
    proposer.load_state_dict(base[proposer_state_key])
    ...
    gate = SpatialRiskGate(
        ...
        float(rbsr.get("initial_gate", base.get("selected_alpha") or 0.25)),
    ).to(device)
    ...
    chunk_signature = sha256_json({
        ...
        "residual_proposer": proposer_kind,
        "confirmation_policy_sha256": confirmation_policy_hash,
        "all_models_freeze_sha256": all_models_freeze_hash,
        "pairs": pairs,
    ...
    proposer_comparator_label = f"{proposer_kind}_clean"
    hybrid_comparator_label = f"hybrid_ridge_{proposer_kind}_validation_selected_clean"
    comparator_row_sets: dict[str, list[dict[str, float | str]]] = {
        "ridge_sourcepca_clean": [],
        proposer_comparator_label: [],
        hybrid_comparator_label: [],
    ...
    global_alpha = float(base.get("selected_alpha") or rbsr.get("initial_gate", 0.25))
    ...
    for start in range(0, len(pairs), args.chunk_pairs):
        ...
        if resumed:
            ...
            with np.load(array_path, allow_pickle=True) as saved:
                ...
                cvae_chunk = np.asarray(saved[proposer_key], dtype=np.float32)
            ...
            cvae_chunk = predict_field(
                proposer, vertex_features_np, condition[start:stop], is_cvae=proposer_is_cvae
            ).astype(np.float32)
            ...
            if calibration_selected is not None:
                ...
                prediction_chunk = enforce_exact_controls(
                    prediction_chunk,
                    controls[start:stop],
                    handles_np,
                )
            ...
            hybrid_chunk = ridge[start:stop] + global_alpha * (
            ...
            if args.save_base_comparators:
                comparator_predictions = {
                    ...
                    proposer_comparator_label: cvae_chunk,
                    hybrid_comparator_label: hybrid_chunk,
                }
            ...
            atomic_savez_compressed(
                ...
                proposer_pred=cvae_chunk,
                residual_proposer=np.asarray([proposer_kind]),
    ...
    if args.save_dense:
        ...
        atomic_savez_compressed(
            ...
            proposer_pred=cvae_prediction.astype(np.float32),
            residual_proposer=np.asarray([proposer_kind]),
            ...
            selected_alpha=np.asarray([global_alpha], dtype=np.float32),
        )
    ...
    report = {
        ...
        "selection_status": (
            ...
            if args.split == "validation"
                ...
                if projection_selected is not None
                    ...
                    (
                        "posthoc_test_after_lamm_reference_dominating_validation_calibration"
                        if calibration_freeze_status == "FROZEN_REFERENCE_DOMINATING_VALIDATION_CALIBRATION"
                        else "one_shot_test_after_strict_ridge_beating_validation_calibration"
                    )
                    if calibration_selected is not None
        ...
        "calibration_freeze_sha256": calibration_freeze_hash,
        "calibration_freeze_status": calibration_freeze_status,
        "calibration_selected": calibration_selected,
        "projection_freeze": (
            None
