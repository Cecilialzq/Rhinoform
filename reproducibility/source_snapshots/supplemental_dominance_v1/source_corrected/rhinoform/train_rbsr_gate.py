...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=["cvae", "field"],
        default="cvae",
        help="Frozen neural proposer whose residual is spatially gated against Ridge.",
    )
    parser.add_argument(
        "--initial-gate",
        type=float,
        default=-1.0,
        help=(
            "Initial gate probability. A negative value reuses the base hybrid alpha when "
            "available, otherwise 0.25."
        ),
    )
    ...
    proposer_state_key = f"{args.residual_proposer}_state_dict"
    if proposer_state_key not in base:
        raise RuntimeError(
            f"Base package does not contain the requested {args.residual_proposer} residual proposer"
        )
    ...
    base_args = base["args"]
    if args.residual_proposer == "cvae":
    ...
    print(f"Caching frozen {args.residual_proposer} train/validation predictions", flush=True)
    proposer_train = predict_field(
        proposer,
        vertex_features_np,
        cond_train,
        is_cvae=proposer_is_cvae,
        progress_label=f"RBSR GATE frozen {args.residual_proposer} train cache",
    ).astype(np.float32)
    proposer_val = predict_field(
        proposer,
        vertex_features_np,
        cond_val,
        is_cvae=proposer_is_cvae,
        progress_label=f"RBSR GATE frozen {args.residual_proposer} validation cache",
    ).astype(np.float32)
    proposer.to("cpu")
    del proposer
    if device.type == "mps":
    ...
    base_alpha = base.get("selected_alpha")
    initial_gate = float(args.initial_gate)
    if initial_gate < 0.0:
        initial_gate = float(base_alpha) if base_alpha is not None else 0.25
    if not 0.0 < initial_gate < 1.0:
        raise ValueError("--initial-gate must lie strictly between zero and one")
    gate_model = SpatialRiskGate(
        vertex_features_np.shape[1], cond_train.shape[1], args.hidden, initial_gate
    ).to(device)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch_index, (indices_cpu,) in enumerate(loader, start=1):
            ...
            neural = torch.as_tensor(proposer_train[indices], dtype=torch.float32, device=device).reshape(len(indices), -1, 3)
    ...
    package = {
        ...
        "residual_proposer": args.residual_proposer,
