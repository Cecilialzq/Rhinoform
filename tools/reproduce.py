...
PAPER_MANIFEST = "reproducibility/PAPER_EXPERIMENT_MANIFEST.json"
FIGURE_MANIFEST = (
    "results/rbsr_final_rerun_holdout_v1/posthoc_subunit_analysis_v1/"
    "figures/FIGURE_EVIDENCE.json"
)
RUNTIME_CHECKSUMS = "benchmarks/runtime/SHA256SUMS"
...
def _verify_runtime_benchmarks(repo_root: Path) -> dict[str, object]:
    checksum_path = repo_root / RUNTIME_CHECKSUMS
    checked = 0
    for line in checksum_path.read_text(encoding="utf-8").splitlines():
        ...
        if not target.is_file() or sha256_file(target) != expected:
            raise RuntimeError(f"Runtime benchmark checksum mismatch: {target}")
        checked += 1
    if checked == 0:
        raise RuntimeError("Runtime benchmark checksum file is empty")
    return {"status": "PASS_RUNTIME_BENCHMARK_HASHES", "files": checked}


def _verify_publication_figures(repo_root: Path) -> dict[str, object]:
    manifest_path = repo_root / FIGURE_MANIFEST
    if not valid_sha256_sidecar(manifest_path):
        raise RuntimeError("Figure evidence manifest or SHA-256 sidecar is invalid")
    record = json.loads(manifest_path.read_text(encoding="utf-8"))
    if record.get("status") != "COMPLETE_HASHED_PUBLICATION_FIGURES":
        raise RuntimeError("Unexpected publication-figure evidence status")
    for relative, expected in record.get("outputs", {}).items():
        target = manifest_path.parent / relative
        if not target.is_file() or sha256_file(target) != expected:
            raise RuntimeError(f"Publication figure hash mismatch: {target}")
    return {
        "status": "PASS_PUBLICATION_FIGURES",
        "files": len(record.get("outputs", {})),
    }


def _verify_executed_notebook(path: Path) -> dict[str, object]:
    record = json.loads(path.read_text(encoding="utf-8"))
    code_cells = [cell for cell in record.get("cells", []) if cell.get("cell_type") == "code"]
    executed = [
        cell
        for cell in code_cells
        if cell.get("execution_count") is not None or cell.get("outputs")
    ]
    if not code_cells or not executed:
        raise RuntimeError(f"Notebook is not an executed experiment record: {path}")
    return {"path": path.relative_to(path.parents[1]).as_posix(), "executed_cells": len(executed)}


def verify_paper_experiments(paths: ReproductionPaths) -> dict[str, object]:
    manifest_path = paths.repo_root / PAPER_MANIFEST
    if not valid_sha256_sidecar(manifest_path):
        raise RuntimeError("Paper experiment manifest or SHA-256 sidecar is invalid")
    record = json.loads(manifest_path.read_text(encoding="utf-8"))
    if record.get("status") != "COMPLETE_PAPER_EXPERIMENT_COVERAGE":
        raise RuntimeError("Unexpected paper experiment manifest status")

    required: set[str] = set(record.get("historical_source_snapshots", []))
    required.add(str(record["restricted_inputs"]))
    notebooks: set[str] = set()
    for experiment in record.get("experiments", []):
        required.update(str(path) for path in experiment.get("code", []))
        required.update(str(path) for path in experiment.get("protocol_and_results", []))
        notebook = experiment.get("notebook")
        if notebook:
            notebooks.add(str(notebook))
            required.add(str(notebook))
    missing = sorted(relative for relative in required if not (paths.repo_root / relative).is_file())
    if missing:
        raise RuntimeError(f"Paper experiments have missing repository paths: {missing}")

    sidecars = 0
    for relative in sorted(required):
        path = paths.repo_root / relative
        if path.with_name(path.name + ".sha256.json").is_file():
            if not valid_sha256_sidecar(path):
                raise RuntimeError(f"Invalid paper-evidence SHA-256 sidecar: {path}")
            sidecars += 1
    notebook_reports = [
        _verify_executed_notebook(paths.repo_root / relative)
        for relative in sorted(notebooks)
    ]
    return {
        "status": "PASS_PAPER_EXPERIMENT_COVERAGE",
        "experiments": len(record.get("experiments", [])),
        "required_paths": len(required),
        "verified_sidecars": sidecars,
        "notebooks": notebook_reports,
        "publication_figures": _verify_publication_figures(paths.repo_root),
        "runtime_benchmarks": _verify_runtime_benchmarks(paths.repo_root),
    }
...
def main() -> int:
    ...
    if args.command in {"verify", "verify-assets", "preflight"}:
        ...
        report["paper_experiments"] = verify_paper_experiments(paths)
