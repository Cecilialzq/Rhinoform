...
CORE_DOMINANCE_METRICS = ("roi_rmse", "normal_flip_pct", "edge_strain_p95")
...
def select_reference_dominating_operating_point(
    candidates: list[dict[str, object]],
    reference: dict[str, object],
    *,
    relative_margin: float = 0.0,
) -> tuple[dict[str, object] | None, list[dict[str, object]]]:
    """Select the most robust candidate below a reference on all core metrics.

    ``relative_margin`` is a predeclared safety buffer: ``0.01`` requires every
    candidate metric to be at least one percent lower than the corresponding
    reference metric.  Selection is intended for validation-only model choice.
    """
    if not 0.0 <= float(relative_margin) < 1.0:
        raise ValueError("relative_margin must lie in [0, 1)")
    thresholds = {
        metric: float(reference[metric]) * (1.0 - float(relative_margin))
        for metric in CORE_DOMINANCE_METRICS
    }
    assessed: list[dict[str, object]] = []
    for candidate in candidates:
        row = dict(candidate)
        for metric in CORE_DOMINANCE_METRICS:
            ...
            row[f"delta_vs_reference_{metric}"] = value - float(reference[metric])
            row[f"threshold_{metric}"] = thresholds[metric]
            row[f"beats_reference_{metric}"] = value < thresholds[metric]
            row[f"relative_improvement_vs_reference_{metric}"] = (
                float(reference[metric]) - value
            ) / float(reference[metric])
        row["dominates_reference_core"] = all(
            bool(row[f"beats_reference_{metric}"])
            for metric in CORE_DOMINANCE_METRICS
        )
        row["minimum_core_relative_improvement"] = min(
            float(row[f"relative_improvement_vs_reference_{metric}"])
            for metric in CORE_DOMINANCE_METRICS
        )
        assessed.append(row)
    feasible = [row for row in assessed if bool(row["dominates_reference_core"])]
    if not feasible:
        return None, assessed
    return min(
        feasible,
...
def validate_test_unlock(
    ...
    *,
    ...
    if status not in {
        "FROZEN_STRICT_VALIDATION_CALIBRATION",
        "FROZEN_REFERENCE_DOMINATING_VALIDATION_CALIBRATION",
    }:
    ...
    if not isinstance(selected, dict):
        raise ValueError("Calibration freeze has no selected operating point")
    if status == "FROZEN_STRICT_VALIDATION_CALIBRATION":
        if selected.get("strictly_beats_ridge") is not True:
            raise ValueError("Calibration freeze does not contain a strict Ridge-beating operating point")
    elif selected.get("dominates_reference_core") is not True:
        raise ValueError("Calibration freeze does not dominate its frozen validation reference")
...
def enforce_exact_controls(
    prediction: np.ndarray,
    controls: np.ndarray,
    landmark_indices: np.ndarray,
) -> np.ndarray:
    """Reapply the hard landmark-control contract after residual calibration."""
    flat = np.asarray(prediction)
    landmarks = np.asarray(landmark_indices, dtype=np.int64).reshape(-1)
    if flat.ndim != 2 or flat.shape[1] % 3 != 0:
        raise ValueError("Prediction must have shape [pairs, 3*vertices]")
    vertices = flat.reshape(flat.shape[0], -1, 3).copy()
    if np.any(landmarks < 0) or np.any(landmarks >= vertices.shape[1]):
        raise ValueError("Landmark index is outside the prediction vertex range")
    control_vertices = np.asarray(controls, dtype=flat.dtype)
    if control_vertices.size != flat.shape[0] * len(landmarks) * 3:
        raise ValueError("Controls do not match pair count and landmark count")
    control_vertices = control_vertices.reshape(flat.shape[0], len(landmarks), 3)
    vertices[:, landmarks] = control_vertices
    return vertices.reshape(flat.shape)
