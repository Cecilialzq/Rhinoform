...
def parse_floats(raw: str) -> list[float]:
    ...
    if values.count(0.0) != 1:
        raise ValueError("Attenuations must include the exact zero-residual Ridge identity audit")
    invalid = [value for value in values if value < 0.0 or value >= 1.0]
    if invalid:
        raise ValueError(
            "Projection attenuations must be zero for the Ridge identity audit or lie strictly "
            f"between zero and one; invalid={invalid}"
        )
    return values
...
def main() -> int:
    ...
    parser.add_argument("--attenuations", default="0,0.5,0.75,0.9")
