...
SYSTEM = "demo/src/components/Studio.jsx;demo/src/components/Studio3D.jsx;demo/src/engine/bundle.js;demo/src/engine/certified.js;demo/src/engine/certifiedLocal.js;demo/public/bundle/manifest.json"
METHOD = "rhinoform/train.py;rhinoform/cvae.py;rhinoform/train_rbsr_gate.py;scripts/evaluation/rbsr_ridge_fold_projection.py;demo/src/engine/certifiedLocal.js"
...
def evidence_for(path: str) -> tuple[str, str, str]:
    ...
    if "eval_06_" in name or name == "fig_a_1_qualitative_all.pdf":
        return "deterministic qualitative cases", QUAL, "case freeze/evidence JSON and NPZ hash sidecars committed; private NPZ bytes independently matched those hashes; exact composite absent from repository"
    if "eval_07_" in name:
...
def main() -> None:
    ...
    parser.add_argument("--inventory", type=Path, default=Path(__file__).with_name("figure_asset_inventory.csv"))
    parser.add_argument("--out", type=Path, default=Path(__file__).with_name("figure_evidence_chain.csv"))
